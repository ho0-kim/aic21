...
def get_loss_model(cfg):
    cfg_loss = cfg["loss"]
    if cfg_loss["type"] == "MaxMarginRankingLoss":
        loss = MaxMarginRankingLoss(margin=cfg_loss["args"]["margin"],
                                    fix_norm=cfg_loss["args"]["fix_norm"])
    elif cfg_loss["type"] == "BinaryCrossEntropy":
        loss = nn.BCELoss()
    return loss